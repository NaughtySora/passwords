...
class ScryptOptions {
  ...
  static parseVersion(value) {
    if (!string.valid(value)) {
      throw new TypeError('Version has to be a string');
    }
    return parseFloat(value.replace(NOT_DIGITS, ""));
  }
...
class Scrypt {
  ...
  #deserialize(payload) {
    ...
    return {
      version: ScryptOptions.parseVersion(components[1]),
  ...
  compare(password, hashed) {
    this.#validate(password);
    const deserialized = this.#deserialize(hashed);
    const salt = deserialized.salt;
    const hash = deserialized.hash;
